...
颜色识别夹取木块
实验效果：
    将红绿蓝其中任意一种颜色的木块放置在小车前方摄像头能看到的范围内，OpenMV会根据识别到的木块颜色来将
    对应的木块放置到对应位置：
        如果识别到的是红色木块，则夹取红色木块放置到前方偏远位置；
        如果识别到的是绿色木块，则夹取绿色木块放置到左侧偏远位置；
        如果识别到的是蓝色木块，则夹取蓝色木块放置到右侧偏远位置；
        未识别到红绿蓝木块时，保持不动。
...
img_w_mm = 240        # 画面宽（即在识别位置start_position_cmd时，画面照射到的实际宽度，单位为mm）
img_h_mm = 165        # 画面高（即在识别位置start_position_cmd时，画面照射到的实际高度，单位为mm）

kms_x_bias = 15       # 逆运动学目标位置x坐标的偏差
kms_y_bias = 30       # 逆运动学目标位置y坐标的偏差
...
start_position_cmd = '$KMS:0,90,90,1500!' # 机械臂运行到识别位置的指令
claw_open_cmd = '#005P0600T1000!'           # 爪子张开指令
claw_close_cmd = '#005P2400T1400!'          # 爪子闭合指令
clamp_after_cmd = '$KMS:0,150,100,1500!'    # 夹取到木块后，机械臂抬起来一些的指令
red_place_cmd = '$KMS:0,300,40,2000!'       # 机械臂运行到放置红色木块位置的指令
green_place_cmd = '$KMS:-200,40,15,2000!'   # 机械臂运行到放置绿色木块位置的指令
blue_place_cmd = '$KMS:200,40,15,2000!'     # 机械臂运行到放置蓝色木块位置的指令

# 设置颜色阈值，此处设置红绿蓝三种颜色的阈值
thresholds = {
              'red': (0, 86, 10, 118, 3, 118),     # 红色阈值(18, 74, 34, 127, -25, 127)
...
def clamp_wood(cx, cy, c_angle):
    ...
    # 串口发送机械臂运行到夹取位置的指令
    if kms_y <= img_to_car_mm + img_h_mm/2 + kms_y_bias:
        uart3_send('$KMS:%s,%s,15,1500!' % (kms_x, kms_y))
    else:
        uart3_send('$KMS:%s,%s,25,1500!' % (kms_x, kms_y))
    utime.sleep(2)                         # 延时以便让动作执行完成
    # 爪子旋转至合适的角度准备进行夹取
...
# 放置木块
def place_wood(target_threshold):
    print('uart3 send color is ', target_threshold)
    if target_threshold == 'red':
        uart3_send(red_place_cmd)          # 串口发送机械臂运行到放置红色木块位置的指令
        color_appear_count['red'] = 0
    elif target_threshold == 'green':
        uart3_send(green_place_cmd)        # 串口发送机械臂运行到放置绿色木块位置的指令
        color_appear_count['green'] = 0
    elif target_threshold == 'blue':
        uart3_send(blue_place_cmd)         # 串口发送机械臂运行到放置蓝色木块位置的指令
        color_appear_count['blue'] = 0
    utime.sleep(3)                         # 延时以便让动作执行完成
