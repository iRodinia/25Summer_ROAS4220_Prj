...
实验用到的二维码（可以将二维码贴到木块上来使用）：
    ...
    第三个二维码内容为QR3
实验效果：
    将贴有上述三个二维码中任意一个的木块放置在小车前方摄像头能看到的范围内，OpenMV会根据识别到的二维码内容来将
    对应的木块放置到对应位置：
        如果识别到的二维码内容是QR1，则夹取该木块放置到前方偏远位置；
        如果识别到的二维码内容是QR2，则夹取该木块放置到左侧偏远位置；
        如果识别到的二维码内容是QR3，则夹取该木块放置到右侧偏远位置；
        未识别到二维码时，保持不动。
...
qr1_place_cmd = '$KMS:0,300,40,2000!'       # 机械臂运行到放置贴有一号二维码木块位置的指令
qr2_place_cmd = '$KMS:-200,40,15,2000!'     # 机械臂运行到放置贴有二号二维码木块位置的指令
qr3_place_cmd = '$KMS:200,40,15,2000!'      # 机械臂运行到放置贴有三号二维码木块位置的指令
...
# 放置木块
def place_wood(target_qr):
    print('uart3 send qr is ', target_qr)
    if target_qr == qr1_content:
        uart3_send(qr1_place_cmd)          # 串口发送机械臂运行到放置贴有一号二维码木块位置的指令
        qr_appear_count[qr1_content] = 0
    elif target_qr == qr2_content:
        uart3_send(qr2_place_cmd)          # 串口发送机械臂运行到放置贴有二号二维码木块位置的指令
        qr_appear_count[qr2_content] = 0
    elif target_qr == qr3_content:
        uart3_send(qr3_place_cmd)          # 串口发送机械臂运行到放置贴有三号二维码木块位置的指令
        qr_appear_count[qr3_content] = 0
...
# 主函数
def main():
    global systick_ms_bak_zhuan, zhuan_flag, servo_yuntai_bias
    while 1:                               # 无限循环
